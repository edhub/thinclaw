...
- **prompt caching 目前不可用**：pi-ai 在发送请求时正确附加了 `cache_control: { type: "ephemeral" }` 到
  system prompt 和最后一条 user 消息，但 bianxie.ai 代理在转发到 Anthropic 时会剥除该字段。
  响应的 `usage.cacheRead` 和 `usage.cacheWrite` 均为 0，已通过实测确认。ThinClaw 侧无法修复，
  需等待 bianxie.ai 修复代理层的 cache_control 转发。
