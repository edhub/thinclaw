...
## 阿里百炼 (bailian / DashScope)

阿里百炼通过 OpenAI 兼容端点暴露千问系列与 DeepSeek 模型，浏览器可直连，无 CORS 限制。

### 鉴权

- **OpenAI Completions**：`Authorization: Bearer <DASHSCOPE_API_KEY>`（标准格式，SDK 自动处理）。
  阿里云控制台获取 API Key（`sk-xxx` 格式）。
...
国际/新加坡地域使用：`https://dashscope-intl.aliyuncs.com/compatible-mode/v1`

### 上下文缓存

| 模式 | 说明 | 计费 |
|---|---|---|
| **隐式缓存** | 自动开启，无法关闭，前缀匹配命中 | 命中 Token 按输入单价 **20%** 计费 |
| **显式缓存** | 需在 `content` 中加 `cache_control: {type: "ephemeral"}` | 创建 125%，命中 **10%** |

显式缓存最少 1024 Token，有效期 5 分钟（命中后重置）。  
隐式缓存最少 256 Token，系统定期清理。  
**注意：** ThinClaw 通过 `onPayload` hook 为百炼请求自动注入 `cache_control` 断点（system 消息末尾 + 最后一条 user 消息末尾），实现显式缓存。隐式缓存由百炼服务端自动处理，无需客户端配置。

### 当前已配置模型

| 模型 ID | 显示名 | 备注 |
|---|---|---|
| `qwen3.6-plus` | 千问 3.6 Plus | 支持显式/隐式缓存，thinking 已关闭 |
| `qwen3.5-flash` | 千问 3.5 Flash | 支持显式/隐式缓存，thinking 已关闭 |
...
老张支持 Google Generative AI 原生协议，端点与官方格式相同。

### 鉴权

- **Google GenAI**：URL 查询参数 `?key=$API_KEY`（SDK 默认行为）。
  **注意**：部分老张端点同样不识别 `x-goog-api-key`，ThinClaw 对 `provider === 'laozhang'` 不额外注入
  `Authorization: Bearer`（与 lingyaai 不同，老张 Google 端点接受标准 SDK 鉴权方式）。

### ThinClaw 配置 (laozhang)
...
七牛支持 Anthropic Messages 原生协议，适合 Claude 模型接入。
