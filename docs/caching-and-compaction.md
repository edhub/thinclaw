...
#### pi-ai 内置的缓存断点（`cacheRetention="short"`）

pi-ai 的 `anthropic.js` provider 通过内置的 `cacheRetention="short"` 策略自动管理所有 Anthropic `cache_control` 断点。**ThinClaw 不需要也不会手动注入缓存标记。**

ThinClaw 的 `onPayload` hook（`src/lib/agent/payload.ts`）是纯被动的：它对每次出站请求做一次 `JSON.parse(JSON.stringify(...))` 快照，写入 `capturedPayloads` 供 SessionViewer 调试用，**原样返回 payload，不做任何修改**。

pi-ai 在每次 Anthropic 请求中自动注入三个缓存断点：

| 断点 | 约 tokens | 来源 | 稳定性 |
|---|---|---|---|
| ① | ~600 | System prompt 末尾（pi-ai 内置） | 随 memory 变化而变，且低于 Anthropic 1,024 最低阈值，实际不生效 |
| ② | ~3,500 | 最后一个 tool 定义（pi-ai 内置） | **永远不变 ✓ — 真正的稳定缓存层** |
...
消息历史部分由断点 ③ 提供跨 turn 的增量缓存。ThinClaw 只需保持 system prompt 和 tools 列表稳定，无需额外操作。
...
| `src/lib/agent/payload.ts` | `onPayload()` — 捕获每次出站 LLM 请求的完整 payload 快照，写入 SessionViewer 供调试；不修改 payload |
